...
export function TemplateList({
  ...
  if (templates.length === 0) {
    return (
      <div
        style={{ textAlign: 'center', padding: '2rem', color: 'var(--pf-v6-global--Color--200)' }}
      >
  ...
  const getTemplateDetails = (template: TemplateSuite) => {
    ...
    if (template.metadata?.persona_count) {
      details.push({
        label: 'Personas',
        value: `${template.metadata.persona_count as number} specialized roles`,
      });
    }
    ...
    if (template.metadata?.agent_count) {
      details.push({
        label: 'Agents',
        value: `${template.metadata.agent_count as number} AI assistants`,
      });
    }

    if (template.metadata?.industry) {
      details.push({ label: 'Industry', value: template.metadata.industry as string });
    }

    if (template.metadata?.use_cases) {
      details.push({
        label: 'Use Cases',
        value: (template.metadata.use_cases as string[]).join(', '),
      });
    }
    ...
    if (template.metadata?.compliance) {
      details.push({
        label: 'Compliance',
        value: (template.metadata.compliance as string[]).join(', '),
      });
    }
      ...
      >
        {templates.map((template) => {
          ...
          return (
            ...
            >
              <CardHeader>
                <CardTitle style={{ color: 'var(--text-primary)' }}>{template.name}</CardTitle>
              </CardHeader>
              <CardBody>
                <div style={{ marginBottom: '1rem' }}>
                ...
                >
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    >
                      {String(Number(template.metadata.persona_count) || 0)} personas
                    </Badge>
                    <Badge
                      isRead
                      style={{
                    ...
                    >
                      {String(Number(template.metadata.agent_count) || 0)} agents
                    </Badge>
                    {isDeployed && (
                      <Badge
                        style={{
                ...
                <Button
                  ...
                  style={{
                    ...
                    color: isTemplateDeployed(template)
                      ...
                      : 'white',
                    cursor: isTemplateDeployed(template) ? 'not-allowed' : 'pointer',
                    fontWeight: 'bold',
                    fontSize: '1rem',
                ...
                <Checkbox
                  ...
                  isChecked={
                    selectedTemplate.agents
                      ? selectedAgents.size === selectedTemplate.agents.length
                      : false
                  }
