...
async def create_virtual_assistant(
    ...
    """
    logger.info(f"=== RECEIVED AGENT CREATION REQUEST ===")
    ...
    try:
        sampling_params = {
            ...
            "repeat_penalty": va.repetition_penalty,
        }
        ...
        for i, tool_info in enumerate(va.tools or []):
            ...
            if tool_info.toolgroup_id == "builtin::rag":
                logger.info(f"🔧 TOOL PROCESSING: Identified builtin::rag tool")
                if len(va.knowledge_base_ids or []) > 0:
                    logger.info(
                        f"🔧 TOOL PROCESSING: Found {len(va.knowledge_base_ids)} knowledge base IDs: {va.knowledge_base_ids}"
                    )
                    tool_dict = dict(
                        name="builtin::rag",
                        args={
                    ...
                    logger.info(
                        f"🔧 TOOL PROCESSING: No knowledge base IDs found, adding builtin::rag as string"
                    )
                    tools.append("builtin::rag")
                    logger.info(f"🔧 TOOL PROCESSING: Added builtin::rag as string")
        ...
        logger.info(f"🔧 TOOL PROCESSING: Tool processing complete!")
        logger.info(
        ...
        if toolgroups:
            ...
            logger.info(f"🔧 AGENT CREATION: Separating MCP and regular toolgroups...")
            for i, toolgroup_id in enumerate(toolgroups):
                logger.info(
            ...
            if mcp_toolgroups:
                ...
                # LlamaStack should handle the tool resolution internally
                logger.info(
                    f"🔧 AGENT CREATION: Using MCP toolgroups as-is (no tool resolution needed)"
                )
            ...
            if regular_toolgroups:
                ...
                if "toolgroups" not in agent_config:
                    ...
                    logger.info(
                        f"🔧 AGENT CREATION: Created new 'toolgroups' array for regular toolgroups"
                    )
        ...
        logger.info(f"🚀 AGENT CREATION: === FINAL AGENT CONFIGURATION ===")
        ...
        logger.info(f"🚀 AGENT CREATION: === END FINAL AGENT CONFIG ===")

        try:
            logger.info(
                f"🚀 AGENT CREATION: Attempting to create agent with LlamaStack..."
            )
            logger.info(
                f"🚀 AGENT CREATION: Calling client.agents.create() with agent_config"
            )
            ...
            logger.info(f"✅ AGENT CREATION: Agent created successfully!")
            logger.info(
            ...
            logger.error(f"❌ AGENT CREATION: Failed to create agent with LlamaStack!")
