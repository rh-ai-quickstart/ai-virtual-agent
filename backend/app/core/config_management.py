...
def update_yaml_config(
    ...
    existing_provider = next(
        (p for p in yaml_content["providers"]["inference"] if p.get("provider_id") == provider_id),
        None
    )

    if existing_provider:
        logger.info(f"Provider '{provider_id}' already exists, skipping provider creation")
    else:
    ...
    existing_model = next(
        (m for m in yaml_content["models"]
...
def restart_deployment(namespace: str, deployment_name: str = "ai-virtual-agent") -> None:
