        ...
        for model in models:
            try:
                if model.api_model_type == "llm":
                    ...
                    if hasattr(model, 'provider_id') and str(model.provider_id) in provider_map:
...
async def register_model_provider(model_provider: ModelProviderConfig):
    ...
    """
    try:
        logger.info(f"Registering provider '{model_provider.provider.provider_id}' and model '{model_provider.model_id}'")
        ...
        updated_yaml = update_yaml_config(
            ...
            provider_model_id=model_provider.model_id,  # Use the model_id from frontend
        ...
        logger.info(f"Successfully registered provider and model, deployment restart triggered")
